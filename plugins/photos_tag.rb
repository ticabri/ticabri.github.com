...
<style type="text/css">
  ...
  class PhotoTag < Liquid::Tag
    def initialize(tag_name, markup, tokens)
      if /(?<className>\S.*\s+)?(?<filename>\S+)(?:\s+(?<thumb>\S+))?(?:\s+(?<width>\S+))?(?:\s+(?<title>.+))?/i =~ markup
        @filename = filename
        @thumb = thumb
        @title = title
        @className = className
    ...
    def render(context)
      ...
      if @filename
        "<a href=\"#{p.path_for(@filename)}\" class=\"fancybox\" title=\"#{@title}\"><img width=\"#{@width}\" class=\"#{@className}\" src=\"#{p.thumb_for(@filename,@thumb)}\" alt=\"#{@title}\" /></a>"
      else
